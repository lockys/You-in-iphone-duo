...
![You, in iPhoneDuo 編輯器與右側浮動操作列](docs/images/desktop.png)
...
- 編輯中的預覽大部分捲出畫面時，自動浮成可拖曳的小視窗；縮放、X／Y 與開始時間仍即時更新。拖曳小視窗不會改變裁切值，支援滑鼠、單指與方向鍵移動，位置限制於可視範圍並避開右側操作列。回到原預覽區會自動收回，也能按返回／關閉；關閉後，需先回到原預覽區才會再次自動顯示。
