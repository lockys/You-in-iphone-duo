...
預設「一部影片」使用目前選取素材套用全程；切換「兩部影片」才顯示摺疊／展開兩個素材位置。切換模式保留素材及設定，單影片產生時不傳第二素材。

上傳只將約 5.824 秒的片段轉成相容的靜音 H.264 預覽，不再轉碼完整長片。原始檔與完整 metadata 保留；正式輸出仍由原始檔合成 1080p，不使用低解析度預覽輸出。

`POST /api/preview` 使用 multipart 的 `uploadId`、`startTime`（表單總量上限 32 KiB、不接受檔案），回傳現有 NDJSON 階段事件；完成包含 `previewUrl`、`previewStartTime`、`previewDuration`。`POST /api/upload` 也提供這三個欄位。前端收到舊版沒有 `previewStartTime` 的完整預覽時沿用原本時間同步。

開始時間更新合併 300 ms 內的操作；舊請求取消、過期回應忽略，位置與縮放仍即時。預覽使用片段局部時間，短影片在伺服器循環補足模板長度。來源的原始時長不受預覽長度影響。

每個來源保留最多四個完成片段。磁碟模式刪除淘汰檔，Blob 模式以條件寫入更新 manifest，使用獨立隨機檔名避免跨 instance 覆寫，讀取需驗證素材權限及片段是否仍在清單內。快取使用來源的既有保存政策，失敗／取消只刪除未完成片段，不刪除來源。

量測指令：`npx tsx scripts/benchmark-preview.ts`，三次前後比較的中位數及 FFmpeg 記憶體峰值寫入 `evidence/preview-benchmark.json`。數字僅包含本機預覽轉碼，不含網路、排隊或 Blob I/O，不能視為正式站端到端延遲。
...
- MP4、MOV、WebM、M4V；最多 5 MiB、不限秒數、4K（最大 4096 × 2160 像素數）。檔名、MIME 與實際容器／視訊均驗證。後端 ffprobe 驗證影片資訊，開始時間可選到影片結束前，不設秒數上限。輸出仍固定為模板長度（約 5.84 秒）。
