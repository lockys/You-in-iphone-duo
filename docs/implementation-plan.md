...
**架構：** Next.js App Router 與 Node Route Handler；Busboy 串流 multipart 到私人暫存目錄；FFmpeg 子程序以參數陣列執行。前端 Canvas 色鍵合成，與後端共享 cover、scale、offset 參數。輸入先經 ffprobe 驗證，瀏覽器不支援的手機編碼自動轉成靜音預覽 MP4。

**素材狀態：** 已從使用者提供的 X 貼文下載 8150.mp4，保留原始檔並完成 174 格分析與真實輸出驗證。

## 1. 素材與工具

- [x] 安裝 Next.js 最新穩定版、React、TypeScript、Tailwind、Vitest、Playwright、原生 FFmpeg / ffprobe。
...
- [x] 200 MiB、5 分鐘、解析度上限；隨機目錄及所有權 token；同站檢查；IP/全域速率與並行上限；timeout、取消終止子程序；回收來源、中間與過期成品。
