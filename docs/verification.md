...
## 2026-09-12：單影片模式、介面重整與片段預覽

- 預設一部影片，兩部模式保留各自素材與構圖；預覽移到首位，使用白／淺灰背景、系統字體及藍色操作。手機保留可見的「更換影片」。設計審查的單項手機提示問題已修正，verdict 為 ship（僅該項修正範圍）。
- lint、typecheck、Modern.js Node production build 通過。完整 Vitest 14 檔、109 項通過；最後清理保護調整後，磁碟／Blob 預覽 API 另 11 項通過。
- 最終完整 production Playwright 58 項全數通過（約 8.3 分鐘）：桌機 Chromium、Android 尺寸 Chromium、iPhone 尺寸 WebKit；包含單／雙模式切換、真實合成下載、短預覽更新、排隊取消重試、產生按鈕點擊時機、三語、分享、浮動預覽與觸控。
- WebKit 暫停時更新開始時間曾顯示舊畫面：現在以片段辨識載入狀態、保留模板播放器、等待 seeked 後才繪製；相同像素回歸連續 10 次通過。
- 預覽完成曾使產生按鈕上移 33px，造成點擊丟失；新增按住按鈕跨越完成事件的瀏覽器回歸，將預覽取消按鈕獨立定位。
- 真實雙影片 MP4 通過 ffprobe、完整解碼、1920×1080、H.264、yuv420p、固定模板長度及 faststart。已檢查開頭／中間／結尾影格：紅色摺疊畫面切為藍色展開畫面，手部、邊框與背景保留，螢幕核心沒有綠幕殘留。
- 範圍：本機 Windows production bundle 與原生 FFmpeg；Blob 測試替換遠端儲存，媒體處理仍是真實 FFmpeg。不是實體 iPhone／Android 或正式 Vercel 的驗收。

預覽量測使用相同合成測試素材，各執行三次取中位數。舊流程轉碼完整來源；新流程只準備模板長度的片段，原始 metadata 與正式 1080p／CRF 20 輸出維持不變。

| 測試素材         | 舊預覽 | 新預覽 | FFmpeg 峰值（舊 → 新） |
| ---------------- | -----: | -----: | ---------------------: |
| 短片             | 148 ms | 179 ms |    47,040 → 47,636 KiB |
| 六分鐘測試片     | 530 ms |  94 ms |    41,248 → 38,224 KiB |
| 兩部短片依序處理 | 169 ms | 190 ms |    38,084 → 38,240 KiB |

六分鐘素材的預覽轉碼時間減少約 82%；短片為補足循環片段增加約 20–30 ms。這是低複雜度測試片的本機量測，不含上傳、排隊、Blob I/O 或正式合成，不能套用為所有影片的加速比例。重跑：`npx tsx scripts/benchmark-preview.ts`；原始結果在忽略目錄 `evidence/preview-benchmark.json`。
